...
def chat_with_model(request: ChatRequest) -> Any:
    ...
    try:
        ...
        input = vectorizer.transform([request.input])
        tipo = clf.predict(input)[0]  # pode ser "Busca", "Entrada", "Investimento", etc.

        # 2) detectar ganho/gasto
        money_type = detect_money_type(input)
        ...
        response_outuput = [f"Intenção: {intent}", f"Tipo financeiro: {money_type}"]
        if category:
