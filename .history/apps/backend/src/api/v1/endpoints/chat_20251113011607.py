...
def chat_with_model(request: ChatRequest) -> Any:
    ...
    try:
        ...
        if transaction not in ["busca", "entrada", "indefinido"]:
            # permitir que o modelo retorne outros rótulos, mas normalizar
            transaction = str(transaction)
        if type_transaction not in ["gasto", "ganho", None]:
        ...
        if str(transaction).lower() == "busca":
            ...
            if transaction_proba:
                resposta_base += f" .(Confiança transação: {transaction_proba:.0%},categoria:{category_proba:.0%},tipo{type_proba:.0%})"
        else:
