...
def chat_with_model(request: ChatRequest) -> Any:
    ...
    try:
        ...
        # 3️⃣ Etapa 2 - Se for ENTRADA ou BUSCA, detectar tipo (ganho, gasto, investimento)
        ...
        # se tivermos tipo/categoria detectados, acrescenta na mensagem para ambos busca/entrada
        if type_transaction:
            resposta_base += f" (Tipo detectado: {type_transaction})"
            if category:
                resposta_base += f" na categoria {category}"

        # adicionar informações de confiança quando disponíveis
        confidences = []
        if transaction_proba is not None:
            confidences.append(f"transação: {transaction_proba:.0%}")
        if type_proba is not None:
            confidences.append(f"tipo: {type_proba:.0%}")
        if category_proba is not None:
            confidences.append(f"categoria: {category_proba:.0%}")
        if confidences:
            resposta_base += f" .(Confiança {' ,'.join(confidences)})"
        else:
            resposta_base = "🤔 Não entendi muito bem sua intenção. Pode reformular?"
